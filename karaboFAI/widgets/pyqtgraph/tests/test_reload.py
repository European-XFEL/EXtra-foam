...
def test_reload():
    ...
    remove_cache(mod)
    widgets.pyqtgraph.reload.reloadAll(path, debug=True)
    if py3:
    ...
    oldcfn = widgets.pyqtgraph.reload.getPreviousVersion(c.fn)
    if oldcfn is None:
    ...
    cfn1 = widgets.pyqtgraph.reload.getPreviousVersion(c.fn)
    cfn2 = widgets.pyqtgraph.reload.getPreviousVersion(cfn1)
    ...
    widgets.pyqtgraph.functions.disconnect(c.sig, c.fn)
