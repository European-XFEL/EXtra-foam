...
class TestFileStreamWindow(unittest.TestCase):
    ...
    def testStandAlone(self):
        ...
        with (patch(f"{module}.RunDirectory") as RunDirectory,
            ...
            with patch("extra_foam.gui.windows.file_stream_w.run_info",
                ...
                fpc.assert_called_once_with(100)
    ...
    def testInitProgressControl(self):
        ...
        with patch.object(widget.tidx_progress_br, "reset") as mocked_reset:
            widget.initProgressControl(100)
            self.assertEqual(0, widget.tidx_start_sld.minimum())
            self.assertEqual(99, widget.tidx_start_sld.maximum())
            self.assertEqual(0, widget.tidx_start_sld.value())
            self.assertEqual("0", widget.tidx_start_lb.text())

            self.assertEqual(0, widget.tidx_end_sld.minimum())
            self.assertEqual(99, widget.tidx_end_sld.maximum())
            self.assertEqual(99, widget.tidx_end_sld.value())
            self.assertEqual("99", widget.tidx_end_lb.text())

            self.assertEqual(0, widget.tidx_progress_br.minimum())
            self.assertEqual(99, widget.tidx_progress_br.maximum())
            self.assertEqual(-1, widget.tidx_progress_br.value())
            self.assertEqual(1, mocked_reset.call_count)
            mocked_reset.reset_mock()

            # test set individual sliders
            widget.tidx_start_sld.setValue(50)
            self.assertEqual(50, widget.tidx_progress_br.minimum())
            self.assertEqual("50", widget.tidx_start_lb.text())
            mocked_reset.assert_called_once()
            mocked_reset.reset_mock()
            # test last tidx is set to be smaller than the first one
            widget.tidx_end_sld.setValue(49)
            self.assertEqual(50, widget.tidx_end_sld.value())
            self.assertEqual(50, widget.tidx_progress_br.maximum())
            self.assertEqual("50", widget.tidx_end_lb.text())
            mocked_reset.assert_called_once()
            mocked_reset.reset_mock()

            # test reset
            widget.initProgressControl(0)
            self.assertEqual(-1, widget.tidx_start_sld.minimum())
            self.assertEqual(-1, widget.tidx_start_sld.maximum())
            self.assertEqual(-1, widget.tidx_start_sld.value())
            self.assertEqual("", widget.tidx_start_lb.text())

            self.assertEqual(-1, widget.tidx_end_sld.minimum())
            self.assertEqual(-1, widget.tidx_end_sld.maximum())
            self.assertEqual(-1, widget.tidx_end_sld.value())
            self.assertEqual("", widget.tidx_end_lb.text())

            self.assertEqual(-1, widget.tidx_progress_br.minimum())
            self.assertEqual(-1, widget.tidx_progress_br.maximum())
            self.assertEqual(-2, widget.tidx_progress_br.value())
