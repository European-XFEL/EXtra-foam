...
protected:
  ...
  DSSC_1MGeometry::CenterType center = geom_->assembledCenter();

  size_t np_ = 2; // number of pulses
  size_t mw_ = T::module_shape[1];
  size_t mh_ = T::module_shape[0];
  size_t nm_ = T::n_modules;
  size_t tw_ = T::tile_shape[1];
  size_t th_ = T::tile_shape[0];
};
...
TYPED_TEST(Geometry1M, testAssemblingShapeCheck)
{
  ...
  xt::xtensor<float, 3> dst3 { xt::empty<float>({this->np_, this->shape[0] + 1, this->shape[1]}) };
...
TYPED_TEST(Geometry1M, testIgnoreTileEdge)
{
  ...
  for (size_t bottom=0, top=this->th_ - 1; bottom < this->shape[0]; top += this->th_, bottom += this->th_)
  {
  ...
  for (size_t left = 0, right = this->tw_ - 1; right < this->shape[1]; left += this->tw_, right += this->tw_)
  {
...
{
  // src and dst have different memory cells
  xt::xtensor<float, 3> src1 { xt::ones<float>({this->np_ - 1, this->shape[0], this->shape[1]}) };
  ...
  // dst has incorrect shape
  xt::xtensor<float, 3> src3 { xt::ones<float>({this->np_, this->shape[0], this->shape[1]}) };
